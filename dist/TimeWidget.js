...
function TimeWidget(
  ...
  d3__namespace.select(target).on("TimeWidget", onTimeWidgetEvent);
  d3__namespace.select(target).on("input", onInput);
  ...
  function initBrushesControls() {
    ...
    if (showBrushesControls) {
      d3__namespace.select(groupsElement).insert("h3", ":first-child").text("Groups:");
  ...
  function renderBrushesControls() {
    d3__namespace.select(groupsElement)
      ...
      .each(function (d, i, n) {
        ...
        div.select("input#groupName").on("input", function (evt) {
          ...
          d3__namespace.select(this).style("width", evt.target.value.length + "ch");
        });
    ...
    d3__namespace.select(groupsElement)
      ...
      .each(function () {
        const li = d3__namespace.select(this);
    ...
    gGroupBrushes
      ...
      .on("click", function () {
        let id = d3__namespace.select(this).attr("id").substr("11");
  ...
  function initDomains({ xDataType, fData }) {
    if (!xDomain) {
      xDomain = fixAxis && _this ? _this.extent.x : d3__namespace.extent(fData, x); // Keep same axes as in the first rendering
    }
  ...
  ts.addReferenceCurves = function (curves) {
    ...
    curves.forEach((c) => {
      c.data.sort((a, b) => d3__namespace.ascending(x(a), x(b)));
